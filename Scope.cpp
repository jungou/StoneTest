...
CScope::CScope()
{
	...
	m_FontTitle.CreateFont(18,0,0,0,FW_BOLD,FALSE,FALSE,FALSE,DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH,CString("Arial"));
	m_FontAxis.CreateFont(15,0,0,0,FW_THIN,FALSE,FALSE,FALSE,DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH,CString("Arial"));
	//m_FontValue.CreatePointFont(120,"Times New Roman");
...
void CScope::DrawCurvePanel(CDC *pDC)//画外框
{
	CPen *pOldPen;
...
void CScope::DrawTrack(CDC *pDC,BOOL bPressed)//图上面工具条操作
{
	CPen *pOldPen;
...
void CScope::DrawAxis(CDC *pDC)
{
	...
	if(m_dDimY<1)
	  ...
	  { pDC->DrawText(CString("右在城城"),CRect(m_rectCurve.left-2,nPosY,m_rectCurve.left+20,nPosY+20),  DT_TOP|DT_LEFT   ); } 
	else
	...
	for(i=1;i<=4;i++)
	{
		...
		if(nPosY<=m_rectCurve.bottom)
		{
			if(m_dDimY<1)				 {sprintf(ary,"%.1f",-i*m_dDimY); str=ary;} 
				//str.Format("%.1f",-i*m_dDimY);   //guo_del
			else if(m_dDimY<1000)        {sprintf(ary,"%.0f",-i*m_dDimY); str=ary;} 
				//str.Format("%.0f",-i*m_dDimY);    //guo_del
			else    {sprintf(ary,"%.0fK",-i*m_dDimY/1000); str=ary;} 
				// str.Format("%.0fK",-i*m_dDimY/1000);     //guo_del
			//pDC->TextOut(m_rectCurve.left-2,nPosY,str);    //guo_del
		}
	}
...
void CScope::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	{
		m_nArrowIndex =0;
		DrawArrow(&dc,m_nArrowIndex,TRUE);//鼠标移到箭头上面，颜色显示为绿色
	}
